...
Lab 04~11에서 만든 `AGENTS.md`와 기준 문서는 변경 전 읽기 순서를 강제합니다. Lab 12에서는 이 기준을 실제 코드 변경에 사용했습니다. 이번 Lab은 그 안전장치를 잠시 건너뛰었을 때 생기는 실패를 관찰해, 앞선 문서가 단순 설명이 아니라 **환각 억제 장치**였음을 역으로 확인합니다.
...
| --- | --- | --- | --- | ---: |
| E1 | 안전장치와 존재 검증 | 세 단계 중 하나로 존재하지 않는 참조를 재현 | 커밋하지 않은 실패 패치 | 10분 |
...
```mermaid
flowchart LR
  A["기준이 읽기 순서를 강제"] --> B["E1에서 안전장치를 의도적으로 생략"]
...
현재 `dependencies`에는 `express`만 있습니다. `orders`의 실제 저장 구조와 메서드는 `src/store.ts`에서 확인할 수 있습니다. E1에서는 검증 장치를 의도적으로 건너뛰어 실패를 만들고, 존재 판정은 패치가 나온 뒤 원문에서 수행합니다.
...
2. **없는 내부 helper** — 1에서 실패 패치가 나오지 않았을 때만 시도합니다.
