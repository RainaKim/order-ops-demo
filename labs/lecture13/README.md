...
Lab 04~11에서 만든 `AGENTS.md`와 기준 문서는 변경 전 읽기 순서를 제공하지만 환각을 완전히 없애지는 않습니다. 이번 Lab은 안전장치가 활성화된 실제 저장소에서 존재하지 않는 외부 모듈 또는 내부 API를 사용한 환각 패치를 재현합니다. 라이브 요청에서 재현되지 않으면 통제된 fixture를 적용해, E1을 반드시 실패 패치가 있는 상태로 끝냅니다.
...
```mermaid
flowchart LR
  A["기준이 읽기 순서를 제공"] --> B["E1에서 환각 패치를 재현"]
...
- `AGENTS.md`의 읽기 순서가 있어도 존재하지 않는 API가 diff에 들어올 수 있는 이유는 무엇인가요?
- 이름이 그럴듯한 것과 코드·패키지에 실제로 존재하는 것은 어떻게 구분하나요?
- 라이브 요청에서 환각이 나오지 않을 때도 fixture로 재현을 완료해야 하는 이유는 무엇인가요?
...
현재 `dependencies`에는 `express`만 있습니다. `orders`의 실제 저장 구조와 메서드는 `src/store.ts`에서 확인할 수 있습니다. 안전장치를 그대로 둔 채 라이브 요청으로 환각 패치 재현을 시도하고, 나오지 않으면 통제된 fixture로 재현을 완료합니다. 존재 판정은 항상 저장소 원문으로 수행합니다.
...
```text
주문 입력 검증을 외부 schema validation 라이브러리를 사용하는 방식으로 정리해줘. dependencies와 lockfile은 바꾸지 말고 src/orders.ts만 수정해라. 검증·stage·commit은 하지 마라.
```

2. **없는 내부 helper** — 1에서 실패 패치가 나오지 않았을 때만 시도합니다. 특정 식별자 이름을 주지 않고 저장 계층에 helper가 있다는 전제로 환각 패치 재현을 시도합니다.

```text
결제 성공 처리를 저장 계층의 상태 전환 helper를 재사용하는 방식으로 정리해줘. 새 helper는 만들지 말고 src/payments.ts만 수정해라. 검증·stage·commit은 하지 마라.
```
...
- [ ] 라이브 요청 또는 fixture를 통해 존재하지 않는 참조가 포함된 패치가 실제로 재현됐는가?
